...
module.exports = CoreObject.extend({
  init: function() {
    ...
    this.client = this.s3.createClient({
      maxAsyncS3: 1,
      s3Options: this.config.get('assets')
    });
  },
  ...
  getUploadParams: function() {
    return {
      ...
      s3Params: {
        ACL: 'public-read',
        Bucket: this.config.get('assets.bucket'),
        Prefix: this.config.get('assets.prefix') || '',
  ...
  getS3Params: function(localFile, stat, callback) {
    var gzipExtensions = this.config.get('assets.gzipExtensions');
